...
namespace ChessCompStompWithHacksLibrary
{
	...
	public class SoundVolumePicker
	{
		...
		public SoundVolumePicker(int xPos, int yPos, int initialVolume, int scalingFactor)
		{
			this._xPos = xPos;
			this._yPos = yPos;
		...
		{
			if (this._currentVolume > 0)
				displayOutput.DrawImageRotatedClockwise(GameImage.SoundOn, x: this._xPos, y: this._yPos, degreesScaled: 0, scalingFactorScaled: 128 * this._scalingFactor / 2);
			else
				displayOutput.DrawImageRotatedClockwise(GameImage.SoundOff, x: this._xPos, y: this._yPos, degreesScaled: 0, scalingFactorScaled: 128 * this._scalingFactor / 2);
