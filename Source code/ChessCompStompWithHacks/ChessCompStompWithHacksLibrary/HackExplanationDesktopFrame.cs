...
namespace ChessCompStompWithHacksLibrary
{
	...
	public class HackExplanationDesktopFrame : IFrame<GameImage, GameFont, GameSound, GameMusic>
	{
		...
		private const int CROSS_ICON_SCALING_FACTOR_SCALED = 128 / 4;

		private const int HACK_EXPLANATION_PANEL_X = 50;
		private const int HACK_EXPLANATION_PANEL_Y = 50;

		private const int HACK_EXPLANATION_PANEL_WIDTH = GlobalConstants.DESKTOP_WINDOW_WIDTH - HACK_EXPLANATION_PANEL_X - HACK_EXPLANATION_PANEL_X;
		private const int HACK_EXPLANATION_PANEL_HEIGHT = GlobalConstants.DESKTOP_WINDOW_HEIGHT - HACK_EXPLANATION_PANEL_Y - HACK_EXPLANATION_PANEL_Y;

		public HackExplanationDesktopFrame(
		...
		public IFrame<GameImage, GameFont, GameSound, GameMusic> ProcessDisplayType(DisplayType displayType, IDisplayProcessing<GameImage> displayProcessing)
		{
			if (displayType != DisplayType.Desktop)
			{
				...
				return new HackExplanationMobileFrame(globalState: this.globalState, sessionState: this.sessionState, hack: this.hack, underlyingFrame: this.underlyingFrame);
			}
		...
		public void Render(IDisplayOutput<GameImage, GameFont> displayOutput)
		{
			...
			this.hackExplanation.Render(displayOutput: translatedDisplayOutput, isMobileDisplayType: false);
