...
namespace ChessCompStompWithHacksEngine
{
	...
	public class AlphaBetaAI : IChessAI
	{
		...
		public AlphaBetaAI(GameState gameState, ITimer timer, IDTRandom random, IDTLogger logger)
		{
			...
			this.random = random;
			this.logger = logger;
			this.boardEvaluator = new StandardBoardEvaluator(random: random);
			...
			if (result.GameStatus == ComputeMoves.GameStatus.InProgress)
			{
				...
				List<Move> nonPawnAndCapturingMoves = new List<Move>();
				for (int i = 0; i < moves.Count; i++)
				{
					if (moves[i].IsPawnMove(originalBoard: gameState.Board) || moves[i].IsCapturingMove(originalBoard: gameState.Board))
						this.topLevelMoves.Add(moves[i]);
					else
						nonPawnAndCapturingMoves.Add(moves[i]);
				}
